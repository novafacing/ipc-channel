...
impl OsIpcSender {
    ...
    fn first_fragment_size(sendbuf_size: usize) -> usize {
        (Self::fragment_size(sendbuf_size) - mem::size_of::<Header>()) & (!8usize + 1)
...
fn recv(
    ...
    UnixError,
> {
    let (mut channels, mut shared_memory_regions, mut descriptors) =
    ...
    unsafe {
        ...
        assert_eq!(
            header.channel_fd_num + header.shared_memory_fd_num + header.descriptor_num,
            channel_length
        );
        for index in 0..header.channel_fd_num {
            channels.push(OsOpaqueIpcChannel::from_fd(
                *cmsg_fds.offset(index as isize),
            ));
        }

        for index in header.channel_fd_num..(header.channel_fd_num + header.shared_memory_fd_num) {
            shared_memory_regions
                .push(OsIpcSharedMemory::from_fd(*cmsg_fds.offset(index as isize)));
        }

        for index in (header.channel_fd_num + header.shared_memory_fd_num)
            ..(header.channel_fd_num + header.shared_memory_fd_num + header.descriptor_num)
        {
